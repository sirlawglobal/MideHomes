...
export default function DashboardMessagesPage() {
    ...
    useEffect(() => {
        fetchMessages(user?.role !== 'superadmin' ? { agentId: user?.id } : {});
        fetchListings();
    }, [fetchMessages, fetchListings, user]);

    // Super Admins see all, regular Admins only see their own listings' messages
    const visibleMessages = messages; // Filtered by API already if filters passed above
            ...
            <div className="flex-1 bg-white rounded-xl shadow-sm border overflow-hidden flex flex-col md:flex-row">
                ...
                <div className="w-full md:w-1/3 xl:w-1/4 border-r flex flex-col">
                    <div className="p-4 border-b bg-slate-50 font-semibold text-slate-700 flex justify-between items-center">
                        All Messages
                        <Badge variant="secondary">{isLoading ? '...' : visibleMessages.length}</Badge>
                    </div>
                                ...
                                {visibleMessages.map(msg => (
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-1">
                                            <span className={`font-semibold ${msg.status === 'Unread' ? 'text-slate-900' : 'text-slate-700'}`}>
                                                {msg.senderName}
                                            </span>
                                        ...
                                        <p className={`text-sm line-clamp-2 ${msg.status === 'Unread' ? 'text-slate-900 font-medium' : 'text-slate-500'}`}>
                                            {msg.message}
                                        </p>
                                        {msg.status === 'Unread' && (
                        ...
                        <>
                            ...
                            <div className="p-6 border-b bg-white flex justify-between items-start">
                                ...
                                <div className="flex gap-2">
                                    <Button variant="outline" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50" onClick={() => handleDelete(selectedMsg.id)}>
