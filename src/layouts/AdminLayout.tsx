...
const SIDEBAR_ITEMS = [
    ...
    { icon: Tags, label: 'Categories', href: '/admin/categories', roles: ['superadmin'] },
...
export function AdminLayout() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex">
            ...
            {/* Sidebar */}
            <aside
                className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white border-r transform transition-transform duration-200 ease-in-out flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
                    }`}
            >
                <div className="h-16 flex items-center px-6 border-b justify-between lg:justify-start">
                    <Link to="/" className="flex items-center gap-2 font-bold text-xl text-blue-900">
                        ...
                        <span>Mide<span className="text-sky-500">Homes</span> Admin</span>
                    </Link>
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <header className="h-16 bg-white border-b flex items-center justify-between px-4 lg:px-8">
                    ...
                    <div className="flex-1 flex justify-end items-center gap-4">
                        <span className="hidden sm:inline-block text-sm text-slate-500">Welcome back, {user?.name}</span>
